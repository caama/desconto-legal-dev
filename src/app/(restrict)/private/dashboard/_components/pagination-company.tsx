...
interface PaginationCompaniesProps {
  page: number
  totalCount: number
  onPageChange: (page: number) => Promise<void> | void
}

export function PaginationCompanies({ page, totalCount, onPageChange }: PaginationCompaniesProps) {
  const perPage = 8
  const pages = Math.ceil(totalCount / perPage) || 1

  return (
    <div className="flex items-center justify-between">
      <span className="font-medium text-muted-foreground text-xs md:text-sm">Total de empresas: {totalCount}</span>
