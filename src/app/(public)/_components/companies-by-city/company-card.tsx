...
export function CompanyCard({ company }: CompanyCardProps) {
  return (
    <Link href={`/city/${company.city.slug}/companies/${company.slug}/details`}>
      <Card className="group hover:-translate-y-1.25 flex h-full flex-col overflow-hidden border-slate-200/80 bg-white p-0 transition-all duration-300 hover:border-[#22254D] hover:shadow-lg">
        ...
        <CardHeader className="space-y-1 px-4 sm:px-5">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              <CardTitle className="text-base leading-tight group-hover:text-[#22254D] sm:text-lg">{company.name}</CardTitle>
        ...
        <CardContent className="flex flex-1 flex-col px-4 pb-4 sm:px-5 sm:pb-5">
          ...
          {company.discount && (
            <ShinyButton className="mt-auto w-full rounded-xl bg-[#22254D] px-4 py-2.5 text-center sm:py-3">
